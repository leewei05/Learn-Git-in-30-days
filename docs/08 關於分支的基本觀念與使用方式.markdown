...
章節整理
-------
1. 建立分支 : git branch [BranchName]

2. 切換分支 : git checkout [BranchName]
(git checkout -b [BranchName] 建立分支並且切換到該分支)

3. 刪除分支 : git branch -d [BranchName] (必須先切換到其他分支)

4. detached HEAD : 目前工作目錄不在最新版，執行commit 會無法追蹤新版本。如果要在detached HEAD 狀態建立可追蹤的版本，正確的方法是透過建立分支來追蹤。
