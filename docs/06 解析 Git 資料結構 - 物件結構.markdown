...
章節整理：
---------
1. 物件如何產生？ 執行 git add, git commit指令的同時，.git/objects的資料夾裡面會新增一個資料夾。資料夾的內容會根據檔案裡面的內容進行hash加密，前兩個位元會成為資料夾名稱，剩下的部分會變成檔名。檔案內容是經過zlib演算法進行壓縮。

2. blob 只存檔案內容，檔案名稱 ex: a.txt 存在 tree 裡頭。

3. 假如有兩個檔案內容相同，在執行 git add 的時候不會產生新的資料夾，會存在同一個資料夾的blob裡頭。

4. Git的優點

		a. 有效率

		b. 歷史紀錄保護：很難被竄改，每個物件都有關聯性。
